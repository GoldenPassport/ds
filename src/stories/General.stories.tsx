...
function TokenRow({
  ...
  return (
    <div className="flex items-center gap-4 py-3 border-b border-ink-100 dark:border-ink-700 last:border-0">
      <code className="w-40 shrink-0 text-xs font-mono text-primary-800 dark:text-primary-400">
        {token}
      </code>
      <code className="w-28 shrink-0 text-xs font-mono text-ink-500 dark:text-ink-300">
        {value}
      </code>
      {description && (
        <p className="text-xs font-body text-ink-500 dark:text-ink-300">{description}</p>
      )}
    </div>
  );
}
        ...
        <div className="flex flex-col gap-3">
          {[
            ...
            { bp: 'lg', px: '1024px', rem: '64rem', desc: 'Laptops, desktops' },
            {
              ...
              px: '1280px',
              rem: '80rem',
              desc: 'Large desktops (default PageContainer max)',
            },
            { bp: '2xl', px: '1536px', rem: '96rem', desc: 'Extra-large screens' },
          ].map(({ bp, px, rem, desc }) => (
            <div key={bp} className="flex items-center gap-4">
              <code className="w-10 shrink-0 text-sm font-mono font-semibold text-ink-900 dark:text-ink-50">
                {bp}
              </code>
              <code className="w-16 shrink-0 text-xs font-mono text-primary-800 dark:text-primary-400">
                {px}
              </code>
              <code className="w-16 shrink-0 text-xs font-mono text-ink-500 dark:text-ink-300">
                {rem}
              </code>
              <div
              ...
              />
              <p className="text-xs font-body text-ink-500 dark:text-ink-300 hidden lg:block">
                {desc}
              </p>
            </div>
          ))}
        ...
        <div className="flex flex-wrap gap-6 items-end">
          {[
            ...
            <div key={name} className="flex flex-col items-center gap-2">
              <div
                className={`w-16 h-16 bg-primary-200 dark:bg-primary-800/50 border-2 border-primary-300 dark:border-primary-700 ${cls}`}
              />
              <div className="text-center">
                <p className="text-xs font-mono font-medium text-ink-700 dark:text-ink-200">
                  {name}
                </p>
                <p className="text-xs font-mono text-ink-500 dark:text-ink-300">{value}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      <Section title="Spacing Scale (key steps)">
        <Card>
          {[
            ...
            <div
              key={token}
              className="flex items-center gap-4 py-2.5 border-b border-ink-100 dark:border-ink-700 last:border-0"
            >
              <code className="w-8 shrink-0 text-xs font-mono font-semibold text-ink-700 dark:text-ink-200">
                {token}
              </code>
              <code className="w-16 shrink-0 text-xs font-mono text-primary-800 dark:text-primary-400">
                {value}
      ...
      <Section title="Animations">
        <div className="flex flex-wrap gap-6">
          {[
            ...
            {
              name: 'gp-slide-up',
              cls: 'animate-gp-slide-up',
              desc: '0.25s cubic-bezier(0.16,1,0.3,1)',
            },
            { name: 'gp-pulse', cls: 'animate-gp-pulse', desc: '2s ease-in-out infinite' },
          ].map(({ name, cls, desc }) => (
            <div key={name} className="flex flex-col items-center gap-3">
              <div
                className={`w-16 h-16 rounded-2xl bg-primary-100 dark:bg-primary-900/30 border border-primary-200 dark:border-primary-800 flex items-center justify-center ${cls}`}
              >
              ...
              <div className="text-center">
                ...
                <p className="text-xs font-body text-ink-500 dark:text-ink-300">{desc}</p>
              </div>
            </div>
          ))}
        </div>
