...
export function SectionHeading({
  ...
  const titleSize = Tag === 'h2' ? 'text-base font-semibold' : 'text-sm font-semibold';
    ...
    <div className={className}>
      ...
      <div className="flex items-start justify-between gap-4">
        <div className="min-w-0">
          <Tag
            className={`${titleSize} font-display text-ink-900 dark:text-ink-50 tracking-tight leading-snug`}
          >
            {title}
          </Tag>
          {description && (
            <p className="mt-0.5 text-sm font-body text-ink-500 dark:text-ink-300">{description}</p>
          )}
        </div>
      ...
      {/* Divider */}
      {divider && !hasTabs && <div className="mt-3 border-t border-ink-200 dark:border-ink-700" />}
      {divider && hasTabs && <div className="border-t border-ink-200 dark:border-ink-700" />}
    </div>
  );
}
