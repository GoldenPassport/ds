...
const tokens: Record<NavbarAppearance, Tokens> = {
  light: {
    ...
    searchWrap:
      'bg-ink-50 dark:bg-ink-700 border-ink-200 dark:border-ink-600 focus-within:border-primary-500',
    searchInput:
      'text-ink-900 dark:text-white placeholder:text-ink-500 dark:placeholder:text-ink-400',
    searchIcon: 'text-ink-500 dark:text-ink-300',
    iconBtn:
      'text-ink-500 dark:text-ink-300 hover:text-ink-700 dark:hover:text-ink-100 hover:bg-ink-100 dark:hover:bg-ink-700',
    mobileMenu: 'bg-white dark:bg-ink-800 border-t border-ink-200 dark:border-ink-700',
    mobileLink:
      'text-ink-500 dark:text-ink-300 hover:text-ink-700 dark:hover:text-ink-100 hover:bg-ink-50 dark:hover:bg-ink-700',
    ...
    userBtn: 'hover:bg-ink-50 dark:hover:bg-ink-700',
    ...
    dropdownItemBase: 'text-ink-700 dark:text-ink-200',
...
function NavLink({ item, t }: { item: NavbarItem; t: Tokens }) {
  const base =
    'flex items-center gap-2 px-3 py-2 rounded-xl text-sm font-medium font-body transition-colors';
        ...
        >
          {(user.name || user.email) && (
            <div className={`px-4 py-3 border-b ${t.dropdownDivider}`}>
              {user.name && (
                <p className={`text-xs font-semibold font-body truncate ${t.userName}`}>
                  {user.name}
                </p>
              )}
              {user.email && (
                ...
                <React.Fragment key={i}>
                  ...
                  <HLMenu.Item>
                    {({ active }) => {
                      ...
                      return mi.href ? (
                        <a href={mi.href} className={cls}>
                          {mi.label}
                        </a>
                      ) : (
                        <button type="button" onClick={mi.onClick} className={cls}>
                          {mi.label}
                        </button>
                      );
                    }}
...
function SearchBox({
  ...
  return (
    <div
      className={`flex items-center gap-2 h-9 px-3 rounded-xl border transition-all ${t.searchWrap} ${className}`}
    >
      ...
      >
        <div className={`lg:hidden ${t.mobileMenu}`}>
          <div className="mx-auto max-w-[80rem] px-4 sm:px-6 py-3 flex flex-col gap-1">
            ...
            {items.map((item, i) => {
              const base =
                'flex items-center gap-3 w-full px-3 py-2.5 rounded-xl text-sm font-medium font-body transition-colors text-left';
                ...
                {(user.name || user.email || user.avatarUrl) && (
                  <div className="flex items-center gap-3 px-3 py-2 mb-1">
                    ...
                    <div className="min-w-0">
                      {user.name && (
                        <p className={`text-sm font-semibold font-body truncate ${t.userName}`}>
                          {user.name}
                        </p>
                      )}
                      {user.email && (
                ...
                {user.menuItems?.map((mi, i) => (
                  <React.Fragment key={i}>
                    ...
                    {mi.href ? (
                      <a
                        href={mi.href}
                        className={`flex w-full items-center px-3 py-2 text-sm font-body rounded-xl transition-colors ${t.mobileLink}`}
                      >
                        {mi.label}
                      </a>
                    ) : (
                      <button
                        type="button"
                        onClick={mi.onClick}
                        className={`flex w-full items-center px-3 py-2 text-sm font-body rounded-xl transition-colors text-left ${t.mobileLink}`}
                      >
                        {mi.label}
                      </button>
                    )}
