    ...
    <div className="h-[500px] overflow-y-auto bg-ink-50 dark:bg-ink-900">
      ...
      <div className="px-6 py-4 space-y-4">
        {Array.from({ length: 20 }, (_, i) => (
          ...
          >
            <span className="text-sm font-body text-ink-500 dark:text-ink-400">
              Content row {i + 1}
            </span>
          </div>
        ))}
...
function MasterDemo() {
  ...
  const summary =
    filtered.length === MASTER_ITEMS.length
  ...
  return (
    <div className="bg-ink-50 dark:bg-ink-900 rounded-2xl overflow-hidden border border-ink-200 dark:border-ink-700">
      ...
      <div className="divide-y divide-ink-100 dark:divide-ink-700">
        {filtered.length === 0 ? (
          <p className="px-4 py-8 text-sm font-body text-center text-ink-500 dark:text-ink-400">
            No workflows found
          </p>
        ) : (
          filtered.map((item) => (
