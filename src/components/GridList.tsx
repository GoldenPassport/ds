...
function DefaultCard({ item }: { item: GridListItem }) {
  ...
  const titleNode = interactive ? (
    ...
    <p className="font-semibold text-sm text-ink-900 dark:text-ink-50 leading-snug">{item.title}</p>
  );
              ...
              <Menu
                trigger={
                  <button
                    ...
                    className="inline-flex items-center justify-center w-7 h-7 rounded-xl text-ink-400 hover:text-ink-600 dark:hover:text-ink-200 hover:bg-ink-100 dark:hover:bg-ink-700 transition-colors border-0 bg-transparent cursor-pointer"
                  >
      ...
      {hasFooter && (
        <div className="mt-auto pt-3 border-t border-ink-100 dark:border-ink-700 flex flex-wrap items-center gap-x-4 gap-y-1.5">
          {item.meta!.map((m, i) => (
            <div
              key={i}
              className="flex items-center gap-1.5 text-xs font-body text-ink-500 dark:text-ink-300"
            >
              {m.icon && (
                <span className="shrink-0 text-ink-300 dark:text-ink-600" aria-hidden="true">
                  {m.icon}
                </span>
              )}
              {m.label}
...
export function GridList({ items, columns = 3, renderItem, className = '' }: GridListProps) {
  return (
    <ul
      role="list"
      className={['grid gap-5', colClass[columns], className].filter(Boolean).join(' ')}
    >
      {items.map((item, i) => (
