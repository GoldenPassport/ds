...
function DefaultRow({ item, showLeading }: { item: StackedListItem; showLeading: boolean }) {
  ...
  const inner = (
    ...
    >
      ...
      <div className="flex items-center gap-4 min-w-0">
        {showLeading && item.leading && <div className="shrink-0">{item.leading}</div>}
        <div className="min-w-0">
          <p
            className={[
              'text-sm font-semibold font-body truncate',
              interactive
                ? 'text-ink-900 dark:text-ink-50 group-hover:text-primary-800 dark:group-hover:text-primary-400 transition-colors duration-100'
      ...
      {(item.trailing || item.trailingMeta) && (
        <div className="hidden sm:flex sm:flex-col sm:items-end shrink-0">
          {item.trailing && (
            <div className="text-sm font-body text-ink-700 dark:text-ink-200">{item.trailing}</div>
          )}
          {item.trailingMeta && (
  ...
  if (item.onClick) {
    return (
      <button type="button" onClick={item.onClick} className="group w-full text-left">
        {inner}
      </button>
    );
  }
