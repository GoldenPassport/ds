...
function NavBadge({ value, t }: { value: string | number; t: Tokens }) {
  return (
    <span
      className={`ml-auto shrink-0 min-w-5 px-1.5 py-0.5 text-xs font-medium font-body rounded-full text-center ${t.badge}`}
    >
      {value}
    </span>
  );
...
function NavItem({
  ...
  const baseCls = [
    ...
    item.active ? (depth === 0 ? t.itemActive : t.childActive) : depth === 0 ? t.item : t.childItem,
  ].join(' ');
  ...
  const inner = (
    <>
      {item.icon && depth === 0 && (
        <span
          className={`shrink-0 w-5 h-5 ${item.active ? t.iconActive : t.icon}`}
          aria-hidden="true"
        >
          {item.icon}
        </span>
      ...
      {depth > 0 && (
        <span
          className="shrink-0 w-1.5 h-1.5 rounded-full bg-current opacity-40 ml-1"
          aria-hidden="true"
        />
      )}
              ...
              <>
                <HLMenu.Item>
                  {({ active }) => (
                    <button
                      type="button"
                      className={`flex w-full items-center gap-2.5 px-4 py-2 text-sm font-body ${t.dropdownItem} ${active ? t.dropdownItemHover : ''}`}
                    >
...
export function SidebarNav({
  ...
  return (
    ...
    >
      {/* Logo */}
      {logo && <div className="shrink-0 flex items-center h-16 px-4">{logo}</div>}
      ...
      >
        {groups.map((group, gi) => (
          <div key={gi}>
            {group.label && (
              <p
                className={`mb-1 px-3 text-xs font-semibold font-body uppercase tracking-wider ${t.groupLabel} ${t.sidebar}`}
              >
                {group.label}
              </p>
            )}
      ...
      {/* Footer slot */}
      {footer && <div className={`shrink-0 px-3 py-3 border-t ${t.border}`}>{footer}</div>}
