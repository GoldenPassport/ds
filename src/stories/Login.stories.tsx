...
function SuccessScreen({ title, body, onReset }: { title: string; body: React.ReactNode; onReset: () => void }) {
  return (
...
function StandardLogin() {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    setTimeout(() => {
      setLoading(false);
      if (email && password === 'password') { setDone(true); }
      else { setError('Incorrect email or password. Try password: password'); }
    }, 1200);
  }
  ...
  if (done) {
    return (
      <AuthShell>
        <SuccessScreen
          title="Welcome back!"
          body={<>You're now signed in as <strong className="text-ink-700 dark:text-ink-200">{email}</strong></>}
          onReset={() => { setDone(false); setEmail(''); setPassword(''); }}
        />
      </AuthShell>
    );
  }
...
function OtpLogin() {
  ...
  return (
    <AuthShell>
      <SuccessScreen
        title="You're in!"
        body={<>Signed in as <strong className="text-ink-700 dark:text-ink-200">{masked}</strong></>}
        onReset={() => { setStep('request'); setContact(''); setOtp(''); setError(''); }}
      />
    </AuthShell>
  );
}
...
function TwoFaLogin() {
  ...
  function handleSignIn(e: React.FormEvent) {
    ...
    setTimeout(() => {
      setLoading(false);
      if (email && password === 'password') { setStep('otp'); }
      else { setError('Incorrect email or password. Try password: password'); }
    }, 1000);
  }
    ...
    return (
      <AuthShell>
        ...
        <Fieldset>
          ...
          <Button variant="primary" loading={loading} disabled={otp.length < 6 || loading}
            className="w-full mt-4" onClick={() => handleOtp(otp)}>
  ...
  return (
    <AuthShell>
      <SuccessScreen
        ...
        body={<>Signed in as <strong className="text-ink-700 dark:text-ink-200">{email}</strong> with 2FA.</>}
        onReset={() => { setStep('credentials'); setEmail(''); setPassword(''); setOtp(''); setError(''); }}
      />
    </AuthShell>
  );
}
