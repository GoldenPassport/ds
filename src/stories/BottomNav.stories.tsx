...
import {
  Home, Compass, Heart, User, Search,
  Bike, PalmtreeIcon, Tv2,
  ShoppingBag, MessageCircle, BookOpen,
} from 'lucide-react';
...
function PlaygroundDemo(args: React.ComponentProps<typeof BottomNav>) {
  ...
  return (
    <Demo>
      <BottomNav {...args} items={TRAVEL_ITEMS} activeValue={active} onChange={setActive} fixed={false} />
    </Demo>
  );
}
...
function InteractiveDemo({ items, appearance, dark }: {
  ...
  return (
    <Demo dark={dark}>
      <BottomNav items={items} activeValue={active} onChange={setActive} appearance={appearance} fixed={false} />
    </Demo>
  );
}
...
function InPageDemo() {
  ...
  const labels: Record<string, string> = {
    home: 'Home', explore: 'Explore', messages: 'Messages', saved: 'Saved', profile: 'Profile',
  };
  return (
    <div className="relative h-[600px] bg-ink-50 overflow-hidden flex flex-col max-w-sm mx-auto border-x border-ink-200">
      ...
      <div className="bg-white border-b border-ink-200 px-4 py-3">
        <h1 className="text-base font-semibold font-display text-ink-900 capitalize">{labels[active]}</h1>
      </div>
      {/* Page content */}
      <div className="flex-1 p-4 space-y-3 overflow-hidden">
