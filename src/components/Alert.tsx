...
const variants: Record<AlertVariant, VariantConfig> = {
  info: {
    ...
    outline:
      'bg-white border border-ink-200 border-l-4 border-l-slate-500 dark:bg-ink-800 dark:border-ink-600 dark:border-l-slate-400',
    ...
    actionBtn:
      'text-slate-700 font-semibold hover:text-slate-900 dark:text-slate-300 dark:hover:text-slate-100',
    DefaultIcon: Info,
  },
  success: {
    tinted: 'bg-green-50 border border-green-200 dark:bg-green-900/20 dark:border-green-800',
    outline:
      'bg-white border border-ink-200 border-l-4 border-l-green-500 dark:bg-ink-800 dark:border-ink-600 dark:border-l-green-400',
    ...
    actionBtn:
      'text-green-700 font-semibold hover:text-green-900 dark:text-green-300 dark:hover:text-green-100',
  ...
  warning: {
    ...
    outline:
      'bg-white border border-ink-200 border-l-4 border-l-primary-500 dark:bg-ink-800 dark:border-ink-600 dark:border-l-primary-400',
    ...
    dismiss:
      'text-primary-500 hover:text-primary-700 dark:text-primary-800 dark:hover:text-primary-400',
    actionBtn:
      'text-primary-800 font-semibold hover:text-primary-900 dark:text-primary-300 dark:hover:text-primary-100',
    DefaultIcon: AlertTriangle,
  },
  error: {
    tinted: 'bg-red-50 border border-red-200 dark:bg-red-900/20 dark:border-red-800',
    outline:
      'bg-white border border-ink-200 border-l-4 border-l-red-500 dark:bg-ink-800 dark:border-ink-600 dark:border-l-red-400',
    ...
    actionBtn:
      'text-red-700 font-semibold hover:text-red-900 dark:text-red-300 dark:hover:text-red-100',
    DefaultIcon: XCircle,
  },
};
...
export function Alert({
  ...
  return (
    <div
      ...
      aria-atomic="true"
      className={['rounded-xl p-4', wrapperClass, className].join(' ')}
    >
      <div className="flex gap-3">
        {/* Icon */}
        {resolvedIcon && <span className={['mt-0.5', cfg.icon].join(' ')}>{resolvedIcon}</span>}

        {/* Body */}
        <div className="flex-1 min-w-0">
          {title && (
            <p className={['text-sm font-semibold font-body mb-1', cfg.title].join(' ')}>{title}</p>
          )}
          {children && (
