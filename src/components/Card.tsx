...
export function Card({
  ...
  return (
    ...
    >
      {header && (
        <div
          className={['border-b border-ink-200 dark:border-ink-700', p, headerMuted ? MUTED : '']
            .filter(Boolean)
            .join(' ')}
        >
          {header}
      ...
      <div className={[p, bodyMuted ? MUTED : ''].filter(Boolean).join(' ')}>{children}</div>

      {footer && (
        <div
          className={['border-t border-ink-200 dark:border-ink-700', p, footerMuted ? MUTED : '']
            .filter(Boolean)
            .join(' ')}
        >
          {footer}
