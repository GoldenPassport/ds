...
export function MediaObject({
  ...
  return (
    <div className={['flex', stack, alignCls[align], gapCls[gap], className].filter(Boolean).join(' ')}>
...
export function MediaObjectList({ children, divided = false, className = '' }: MediaObjectListProps) {
  const items = React.Children.toArray(children);
  return (
    <div className={['flex flex-col', divided ? 'divide-y divide-ink-100 dark:divide-ink-700' : 'gap-4', className].join(' ')}>
      {divided
        ? items.map((child, i) => <div key={i} className="py-4 first:pt-0 last:pb-0">{child}</div>)
        : items}
    </div>
  );
}
