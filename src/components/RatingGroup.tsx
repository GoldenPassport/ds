...
function StarIcon({
  ...
  if (state === 'full') {
    return <span className="block w-full h-full text-primary-400">{iconFull}</span>;
  }

  if (state === 'half') {
    // Use provided custom half icon, or clip-path overlay approach
    if (iconHalf) {
      return <span className="block w-full h-full text-primary-400">{iconHalf}</span>;
    }
    return (
      <span className="relative block w-full h-full">
        {/* Muted base */}
        <span className="block w-full h-full text-ink-300 dark:text-ink-600">{iconEmpty}</span>
        {/* Coloured left half */}
        <span
  ...
  // empty
  return <span className="block w-full h-full text-ink-300 dark:text-ink-600">{iconEmpty}</span>;
}
...
export function RatingGroup({
  ...
  // Default icons
  const emptyIcon = iconEmpty ?? <Star className="w-full h-full" strokeWidth={1.5} />;
  const fullIcon = iconFull ?? <Star className="w-full h-full fill-current" strokeWidth={1.5} />;
  ...
  function handleKeyDown(e: React.KeyboardEvent) {
    ...
    if (e.key === 'ArrowRight' || e.key === 'ArrowUp')
      next = Math.min(count, committedValue + step);
  ...
  return (
    ...
    >
      ...
      {label && (
        <label
          htmlFor={id}
          className={['font-medium font-body text-ink-700 dark:text-ink-300', t.label].join(' ')}
        >
          {label}
        </label>
      )}
      ...
      >
        {Array.from({ length: count }, (_, idx) => {
          ...
          const starIcon = (
            <StarIcon state={state} iconEmpty={emptyIcon} iconHalf={iconHalf} iconFull={fullIcon} />
          );

          if (!interactive) {
            return (
              <span key={index} className={`block shrink-0 ${t.star}`} aria-hidden="true">
                {starIcon}
              </span>
            );
          }
          ...
          return (
            <button
              ...
              className={[
                ...
                hoverValue !== null ? (index <= Math.ceil(hoverValue) ? 'scale-110' : '') : '',
              ]
                .filter(Boolean)
                .join(' ')}
