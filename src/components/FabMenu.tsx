...
const triggerColor: Record<FabMenuColor, string> = {
  ...
  surface:
    'bg-white dark:bg-ink-800 text-ink-700 dark:text-ink-100 hover:bg-ink-50 dark:hover:bg-ink-700 shadow-md',
};

// Action pill colour — tinted version of the FAB colour
const itemColor: Record<FabMenuColor, string> = {
  primary:
    'bg-primary-100 dark:bg-primary-900/40 text-primary-900 dark:text-primary-100 hover:bg-primary-200 dark:hover:bg-primary-900/60',
  secondary:
    'bg-ink-200 dark:bg-ink-700 text-ink-800 dark:text-ink-100 hover:bg-ink-300 dark:hover:bg-ink-600',
  surface:
    'bg-white dark:bg-ink-800 text-ink-700 dark:text-ink-100 shadow hover:bg-ink-50 dark:hover:bg-ink-700',
};
...
export function FabMenu({
  ...
  return (
    <>
      {/* Backdrop */}
      {isOpen && <div className={`${posRoot} inset-0 z-40`} aria-hidden="true" onClick={close} />}
      ...
      >
        ...
        <div className={['flex flex-col-reverse gap-2.5', itemAlignCls[position]].join(' ')}>
          {items.map((item, i) => {
            const delay = isOpen ? `${i * 40}ms` : `${(items.length - 1 - i) * 30}ms`;
            ...
            return item.href ? (
              <a key={i} href={item.href} className={pillCls} style={{ transitionDelay: delay }}>
                {inner}
              </a>
            ) : (
              <button
        ...
        >
          ...
          {label && !isOpen && <span className="text-sm">{label}</span>}
        </button>
      </div>
    </>
  );
